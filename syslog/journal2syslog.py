...
def _init_journal_reader() -> journal.Reader:
    """
    Initialize journal reader, handling container boot ID mismatch.

    In containerized environments (like HA add-ons), the container's boot ID
    differs from the host's boot ID. Using this_boot() would filter for the
    container's boot ID which doesn't exist in the host journal, resulting in
    no entries being read.

    Instead, we skip boot ID filtering entirely and just seek to the tail.
    """
    jr = journal.Reader(path="/var/log/journal")

    # NOTE: Intentionally NOT calling jr.this_boot() here.
    # In containers, the boot ID from /proc/sys/kernel/random/boot_id is the
    # container's boot ID, which doesn't match any boot ID in the host's
    # journal (mounted from /var/log/journal). this_boot() would silently
    # filter out ALL entries, causing the add-on to appear working but never
    # forward any logs.
...
def main():
    ...
    while True:
        ...
        for entry in jr:
            ...
            try:
                # SysLogHandler.emit() handles the actual sending, but we
                # bypass it to use our own formatting. Send directly via the
                # handler's socket, recreating it if needed.
